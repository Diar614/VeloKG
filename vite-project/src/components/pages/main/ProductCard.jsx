...
const ProductCard = ({ product, index }) => {
  ...
  return (
    <div
      key={`${id}-${index}`}
      className="bg-white shadow-md rounded-md overflow-hidden mx-2 w-full h-full flex flex-col" 
    >
      {bike1 && (
        <>
...
/>
          ...
          <div className="p-4 bg-stone-200">
            <h1 className="text-sm font-bold">{bike4.name}</h1>
            <p>{bike4.description}</p>
            <div className="mt-2">
              <input type="checkbox" id={`compare-${id}-${index}`} />
              <label htmlFor={`compare-${id}-${index}`} className="ml-1 text-2xs">
                + ДОБАВИТЬ ДЛЯ СРАВНЕНИЯ
